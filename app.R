...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Step 1: Proteomics Data Upload", tabName = "proteomics_upload", icon = icon("upload")),
      menuItem("Step 2: Region-Specific Proteins Extraction", tabName = "region_genes", icon = icon("dna")),
      menuItem("Step 3: scRNA-seq Data Upload", tabName = "scrna_upload", icon = icon("file-import")),
      menuItem("Step 4: scRNA-seq Data Processing", tabName = "scrna_processing", icon = icon("cogs")),
      menuItem("Step 5: Query Method Inplement", tabName = "query_method", icon = icon("search")),
  ...
  dashboardBody(
    ...
    tags$head(
      tags$style(HTML("
        ...
        .info-box { background-color: #d9edf7; padding: 15px; border-radius: 5px; margin: 10px 0; }
        .btn-custom { background-color: #3c8dbc; color: white; margin: 5px; }
        .example-text { color: #777; font-size: 12px; font-style: italic; }
      "))
    ),
    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          box(
            width = 12,
            title = "Welcome to Multi-Omics Integration Pipeline",
            status = "primary",
            solidHeader = TRUE,
            h3("🧬 Spatial Proteomics Integration with scRNA-seq"),
            ...
            tags$ol(
              ...
              tags$li(tags$b("scRNA-seq Upload:"), " Upload your single-cell RNA-seq data (Seurat object)"),
              tags$li(tags$b("scRNA-seq Processing:"), " Process and validate scRNA-seq data with cell type annotations"),
              tags$li(tags$b("Query Method:"), " Compute cell-type enrichment z-scores using region-specific proteins"),
            ...
            actionButton("start_btn", "Get Started →", class = "btn-custom btn-lg", icon = icon("play"))
          )
        )
      ),
      
      # Step 1: Proteomics Upload Tab
      tabItem(
        tabName = "proteomics_upload",
        fluidRow(
          box(
            width = 12,
            title = "Step 1: Upload Spatial Proteomics Data",
            status = "primary",
            solidHeader = TRUE,
            ...
            div(
              class = "upload-box",
              h4("🧬 Spatial Proteomics Expression Matrix", style = "color: #3c8dbc;"),
              p("Upload your spatial proteomics expression matrix (proteins × samples)."),
              
              # Format Example
              div(
                style = "background-color: #f9f9f9; padding: 10px; border-left: 4px solid #9c27b0; margin: 10px 0;",
                p(tags$b("Expected Format:"), style = "margin-bottom: 5px;"),
                tags$table(
                  style = "font-family: monospace; font-size: 11px; border-collapse: collapse;",
                  tags$tr(
                    tags$th("Protein", style = "border: 1px solid #ddd; padding: 4px; background: #f3e5f5;"),
                    tags$th("S1", style = "border: 1px solid #ddd; padding: 4px; background: #f3e5f5;"),
                    tags$th("S2", style = "border: 1px solid #ddd; padding: 4px; background: #f3e5f5;"),
                    tags$th("S3", style = "border: 1px solid #ddd; padding: 4px; background: #f3e5f5;"),
                    tags$th("...", style = "border: 1px solid #ddd; padding: 4px; background: #f3e5f5;")
                  ),
                  tags$tr(
                    tags$td("CD3E", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("12.206", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("13.397", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("12.384", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("...", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("VIM", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("13.963", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("12.083", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("13.189", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("...", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("...", style = "border: 1px solid #ddd; padding: 4px;"),
                ...
                p(class = "example-text", style = "margin-top: 10px;",
                  "• First column: Protein names/IDs, using gene symbol",
                  tags$br(),
                  "• Remaining columns: Sample IDs (matching metadata)",
                  tags$br(),
                  "• Values: Protein abundance (log2 transformed, normalized intensity, no missing values)")
              ),
              
              p(class = "example-text", "Supported formats: .csv, .xlsx, .txt, .tsv"),
              
              # Download Example Data Button
              div(
                style = "margin: 10px 0; padding: 10px; background-color: #fff3cd; border-left: 4px solid #ffc107; border-radius: 3px;",
                p(style = "margin: 0 0 8px 0; font-weight: bold; color: #856404;",
                  icon("lightbulb"), " Want to test with example data?", tags$br(),
                  "Stroma (N = 11) vs Tumor (N = 12) LCM-LC/MS dia-PASEF spatial proteomics data, 6842 protein IDs"),
              ...
              fileInput(
                "spatial_expr_file",
                "Choose Proteomics Expression File:",
                accept = c(".csv", ".xlsx", ".txt", ".tsv", ".xls"),
                multiple = FALSE
              ),
            ...
            div(
              class = "upload-box",
              h4("📋 Sample Metadata", style = "color: #3c8dbc;"),
              p("Upload metadata with sample information and region/group annotations."),
              
              # Format Example
              div(
                style = "background-color: #f9f9f9; padding: 10px; border-left: 4px solid #3c8dbc; margin: 10px 0;",
                p(tags$b("Expected Format:"), style = "margin-bottom: 5px;"),
                tags$table(
                  style = "font-family: monospace; font-size: 11px; border-collapse: collapse;",
                  tags$tr(
                    tags$th("Sample_ID", style = "border: 1px solid #ddd; padding: 4px; background: #e3f2fd;"),
                    tags$th("Sample_group", style = "border: 1px solid #ddd; padding: 4px; background: #e3f2fd;")
                  ),
                  tags$tr(
                    tags$td("S1", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("Tumor", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("S2", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("Tumor", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("S3", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("Tumor", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("S13", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("Stroma", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("S14", style = "border: 1px solid #ddd; padding: 4px;"),
                    tags$td("Stroma", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("...", style = "border: 1px solid #ddd; padding: 4px;"),
                ...
                p(class = "example-text", style = "margin-top: 10px;",
                  "• Column 1: Sample IDs (must match proteomics matrix column order)",
                  tags$br(),
                  "• Column 2: Region/Group labels (e.g., Tumor, Stroma, Epithelium, etc)",
                  tags$br(),
              ...
              # Download Example Metadata Button
              div(
                style = "margin: 10px 0; padding: 10px; background-color: #fff3cd; border-left: 4px solid #ffc107; border-radius: 3px;",
                p(style = "margin: 0 0 8px 0; font-weight: bold; color: #856404;",
                  icon("lightbulb"), " Want to test with example data?"),
                downloadButton("download_example_metadata", 
            ...
            hr(),
            actionButton("validate_proteomics_btn", "Validate Proteomics Data →", class = "btn-custom btn-lg", icon = icon("check"))
          )
        ),
        ...
        fluidRow(
          box(
            width = 12,
            title = "Proteomics Data Validation Summary",
            status = "info",
            solidHeader = TRUE,
      ...
      # Region-Specific Proteins Tab
      tabItem(
        tabName = "region_genes",
        fluidRow(
          box(
            width = 12,
            title = "Step 2: Identify Region-Specific Proteins",
            status = "primary",
            solidHeader = TRUE,
            
            div(
              class = "info-box",
              h4("ℹ️ About This Step:"),
              p("This step identifies proteins that are significantly upregulated in specific histopathological regions 
                using limma moderated t tests. These region-specific proteins will be used in subsequent enrichment analyses.")
            ),
            ...
            fluidRow(
              column(4,
                     selectInput("group_column", "Select Group/Region Column:", choices = NULL)
              ),
              column(4,
                     numericInput("pval_threshold", "Adjusted P Value Threshold:", value = 0.05, min = 0, max = 1, step = 0.01)
              ),
              column(4,
                     numericInput("fc_threshold", "Fold Change Threshold:", value = 2, min = 1, max = 100, step = 0.05)
              )
            ),
            
            actionButton("run_region_analysis_btn", "Run Analysis", class = "btn-custom btn-lg", icon = icon("play")),
            hr(),
            
            uiOutput("region_analysis_status"),
            
            conditionalPanel(
              condition = "output.region_analysis_complete",
              h4("Principal Component Analysis"),
              plotOutput("pca_plot", height = "600px"),
              downloadButton("download_pca_plot", "Download PCA Plot", class = "btn-custom"),
              hr(),
              h4("Region-Specific Protein Overlap Analysis"),
              div(
                class = "info-box",
                p(icon("info-circle"), " The Venn diagram shows overlaps between region-specific proteins. ",
                  "High overlap may indicate the need to adjust FC/p-value thresholds for more specific markers.")
              ),
              plotOutput("venn_plot", height = "600px"),
              verbatimTextOutput("overlap_stats"),
              downloadButton("download_venn_plot", "Download Venn Diagram", class = "btn-custom"),
              hr(),
              h4("Differential Expression Results"),
              DTOutput("region_genes_table"),
              downloadButton("download_region_genes", "Download DE Results", class = "btn-custom")
            )
          )
        )
      ),
      
      # Step 3: scRNA-seq Upload Tab
      tabItem(
        tabName = "scrna_upload",
        fluidRow(
          box(
            width = 12,
            title = "Step 3: Upload scRNA-seq Data (10X Format)",
            status = "primary",
            solidHeader = TRUE,
            ...
            div(
              class = "info-box",
              h4("ℹ️ About This Step:"),
              p("Upload your 10X Genomics scRNA-seq data files. You need 4 files:",
                tags$br(),
                "1. ", tags$b("Sparse matrix file"), " (count_matrix_sparse.mtx)",
                tags$br(),
                "2. ", tags$b("Genes file"), " (count_matrix_genes.tsv)",
                tags$br(),
                "3. ", tags$b("Barcodes file"), " (count_matrix_barcodes.tsv)",
                tags$br(),
                "4. ", tags$b("Metadata file"), " (metadata.csv) with cell type annotations")
            ),
            ...
            div(
              class = "upload-box",
              h4("📊 1. Sparse Matrix File", style = "color: #3c8dbc;"),
              p("Upload the count matrix in Matrix Market format (.mtx)."),
              p(class = "example-text", "This file contains the gene expression counts in sparse matrix format."),
              ...
              conditionalPanel(
                ...
                fileInput(
                  "scrna_matrix_file",
                  "Upload count_matrix_sparse.mtx:",
                  accept = c(".mtx", ".mtx.gz", ".gz")
                )
              ),
              ...
              conditionalPanel(
                ...
                div(
                  style = "background-color: #dff0d8; padding: 10px; margin: 10px 0; border-radius: 3px;",
                  p(style = "margin: 0; font-size: 12px;",
                    icon("lightbulb"), tags$b(" For large files:"),
                    " Provide the full file path on your local machine.",
                    tags$br(),
                    "Example: /Users/yourname/Documents/count_matrix_sparse.mtx")
                ),
                textInput(
                  "scrna_matrix_path",
                  "Enter full path to matrix file:",
                  value = "",
                  placeholder = "/path/to/count_matrix_sparse.mtx",
                  width = "100%"
                )
              )
            ),
            ...
            div(
              class = "upload-box",
              h4("🧬 2. Genes File", style = "color: #3c8dbc;"),
              p("Upload the genes/features file (.tsv)."),
              p(class = "example-text", "This file contains gene IDs and symbols (rows of the matrix)."),
              
              # Format Example for Genes
              div(
                style = "background-color: #f9f9f9; padding: 10px; border-left: 4px solid #4caf50; margin: 10px 0;",
                p(tags$b("Expected Format:"), style = "margin-bottom: 5px;"),
                tags$table(
                  style = "font-family: monospace; font-size: 11px; border-collapse: collapse;",
                  tags$tr(
                    tags$th("Gene.symbol", style = "border: 1px solid #ddd; padding: 4px; background: #e8f5e9;")
                  ),
                  tags$tr(
                    tags$td("RP11-34P13.7", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("FO538757.3", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("AP006222.2", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("RP4-669L17.10", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("LINC00115", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("FAM41C", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("...", style = "border: 1px solid #ddd; padding: 4px;")
                  )
                ),
                p(class = "example-text", style = "margin-top: 10px; font-size: 11px;",
                  "• Single column with gene symbols",
                  tags$br(),
                  "• Header: 'Gene.symbol' or similar",
                  tags$br(),
                  "• Number of rows must match matrix rows")
              ),
              ...
              conditionalPanel(
                ...
                textInput(
                  "scrna_genes_path",
                  "Enter full path to genes file:",
                  value = "",
                  placeholder = "/path/to/count_matrix_genes.tsv",
                  width = "100%"
                )
              )
            ),
            ...
            div(
              class = "upload-box",
              h4("🔖 3. Barcodes File", style = "color: #3c8dbc;"),
              p("Upload the cell barcodes file (.tsv)."),
              p(class = "example-text", "This file contains cell barcodes (columns of the matrix)."),
              
              # Format Example for Barcodes
              div(
                style = "background-color: #f9f9f9; padding: 10px; border-left: 4px solid #2196f3; margin: 10px 0;",
                p(tags$b("Expected Format:"), style = "margin-bottom: 5px;"),
                tags$table(
                  style = "font-family: monospace; font-size: 11px; border-collapse: collapse;",
                  tags$tr(
                    tags$th("barcode", style = "border: 1px solid #ddd; padding: 4px; background: #e3f2fd;")
                  ),
                  tags$tr(
                    tags$td("CID3586_AAGACCTCAGCATGAG", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("CID3586_AAGGTTCGTAGTACCT", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("CID3586_ACCAGTAGTTGTGGCC", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("CID3586_ACCCACTAGATGTCGG", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("CID3586_ACTGATGGTCAACTGT", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("CID3586_ACTTGTTAGGGAACA", style = "border: 1px solid #ddd; padding: 4px;")
                  ),
                  tags$tr(
                    tags$td("...", style = "border: 1px solid #ddd; padding: 4px;")
                  )
                ...
                p(class = "example-text", style = "margin-top: 10px; font-size: 11px;",
                  "• Single column with cell barcodes",
                  tags$br(),
                  "• Header: 'barcode' or similar",
                  tags$br(),
                  "• Format: SampleID_BarcodeSequence, or BarcodeSequence only",
                  tags$br(),
                  "• Number of rows must match matrix columns",
                  tags$br(),
                  "• Must match barcodes in metadata file")
              ),
              
              radioButtons(
                "barcodes_load_method",
                "Choose loading method:",
                choices = c(
                  "Upload file" = "upload",
                  "Load from local path" = "path"
              ...
              conditionalPanel(
                ...
                fileInput(
                  "scrna_barcodes_file",
                  "Upload count_matrix_barcodes.tsv:",
                  accept = c(".tsv", ".txt", ".csv")
                )
              ),
              ...
              conditionalPanel(
                ...
                textInput(
                  "scrna_barcodes_path",
                  "Enter full path to barcodes file:",
                  value = "",
                  placeholder = "/path/to/count_matrix_barcodes.tsv",
                  width = "100%"
                )
              )
            ),
            ...
            div(
              class = "upload-box",
              h4("📋 4. Cell Metadata", style = "color: #3c8dbc;"),
              ...
              div(
                ...
                p(tags$b("Expected Format:"), style = "margin-bottom: 5px;"),
                tags$table(
                  style = "font-family: monospace; font-size: 10px; border-collapse: collapse; width: 100%; overflow-x: auto;",
                  tags$tr(
                    tags$th("barcode", style = "border: 1px solid #ddd; padding: 3px; background: #f3e5f5; font-size: 10px;"),
                    tags$th("orig.ident", style = "border: 1px solid #ddd; padding: 3px; background: #f3e5f5; font-size: 10px;"),
                    tags$th("nCount_RNA", style = "border: 1px solid #ddd; padding: 3px; background: #f3e5f5; font-size: 10px;"),
                    tags$th("nFeature_RNA", style = "border: 1px solid #ddd; padding: 3px; background: #f3e5f5; font-size: 10px;"),
                    tags$th("percent.mito", style = "border: 1px solid #ddd; padding: 3px; background: #f3e5f5; font-size: 10px;"),
                    tags$th("subtype", style = "border: 1px solid #ddd; padding: 3px; background: #f3e5f5; font-size: 10px;"),
                    tags$th("celltype_major", style = "border: 1px solid #ddd; padding: 3px; background: #f3e5f5; font-size: 10px;")
                  ),
                  tags$tr(
                    tags$td("CID3586_AACCATGAGACAGACC", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("CID3586", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("4581", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("1689", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("1.60", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("HER2+", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("Endothelial", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;")
                  ),
                  tags$tr(
                    tags$td("CID3586_AACCATGCACATTAGC", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("CID3586", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("1726", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("779", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("5.79", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("HER2+", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("Endothelial", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;")
                  ),
                  tags$tr(
                    tags$td("CID3586_AACCATGCAGGTCACT", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("CID3586", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("1229", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("514", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("1.38", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("HER2+", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("Endothelial", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;")
                  ),
                  tags$tr(
                    tags$td("...", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("...", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("...", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("...", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                    tags$td("...", style = "border: 1px solid #ddd; padding: 3px; font-size: 10px;"),
                ...
                p(class = "example-text", style = "margin-top: 10px; font-size: 11px;",
                  tags$b("Key Requirements:"),
                  tags$br(),
                  "• ", tags$b("First column:"), " Cell barcodes (must match barcodes file exactly)",
                  tags$br(),
                  "• ", tags$b("Cell type columns:"), " Must include TWO cell type annotation columns",
                  tags$br(),
                  "  (one for crude/major cell types, one for delicate/minor cell types)",
                  tags$br(),
                  "• ", tags$b("Optional columns:"), " QC metrics (nCount_RNA, nFeature_RNA, percent.mito), sample IDs, etc.",
                  tags$br(),
                  tags$br(),
                  tags$b("Common delicate cell type column names:"),
                  tags$br(),
                  "  celltype_minor, celltype_subset, subcluster, annotation",
                  tags$br(),
                  tags$br(),
                  tags$b("Common crude cell type column names:"),
                  tags$br(),
                  "  celltype_major, cell_type, CellType, cluster")
              ),
              
              p(class = "example-text", "Required: Must contain cell type annotations. Supported formats: .csv, .tsv, .txt"),
              
              radioButtons(
                "metadata_load_method",
                "Choose loading method:",
                choices = c(
                  "Upload file" = "upload",
                  "Load from local path" = "path"
              ...
              conditionalPanel(
                ...
                textInput(
                  "scrna_metadata_path",
                  "Enter full path to metadata file:",
                  value = "",
            ...
            hr(),
            actionButton("load_scrna_btn", "Load Raw Data (No Seurat Object Yet) →", class = "btn-custom btn-lg", icon = icon("upload"))
          )
        ),
        ...
        fluidRow(
          box(
            width = 12,
            title = "scRNA-seq Data Loading Progress",
            status = "info",
            solidHeader = TRUE,
      ...
      # Step 4: scRNA-seq Processing Tab
      tabItem(
        tabName = "scrna_processing",
        fluidRow(
          box(
            width = 12,
            title = "Step 4: Preprocess & Filter scRNA-seq Data",
            status = "primary",
            solidHeader = TRUE,
            ...
            div(
              class = "info-box",
              h4("ℹ️ About This Step:"),
              p("Filter and preprocess your scRNA-seq data by:",
                tags$br(),
                "1. Select specific samples by subtype or patient resource (e.g., TNBC, HER2+, normal controls, etc.)",
                tags$br(),
                "2. Filter genes expressed in minimum number of cells, by default n > 10",
                tags$br(),
                "3. Normalize and scale data",
                tags$br(),
                "4. Validate cell type annotations")
            ),
            
            hr(),
            
            h4("1. Sample Selection (Optional)"),
            p("Filter cells by sample subtype or patient resource. Leave blank to keep all samples."),
            fluidRow(
              column(6,
                     selectInput("subtype_column",
                                 "Subtype Column Name:",
                                 choices = NULL,
                                 selected = NULL)
              ),
            ...
            fluidRow(
              column(6,
                     numericInput("min_cells_per_gene",
            ...
            h4("3. Cell Type Annotation Columns"),
            p("Specify the two cell type annotation columns (major and minor)."),
            fluidRow(
              column(6,
                     selectInput("celltype_major_column",
                                 "Major Cell Type Column:",
                                 choices = NULL,
                                 selected = NULL)
              ),
              column(6,
                     selectInput("celltype_minor_column",
                                 "Minor Cell Type Column:",
                                 choices = NULL,
                                 selected = NULL)
              )
            ),
            ...
            actionButton("process_scrna_btn", "Run Preprocessing & Validation →", 
                         class = "btn-custom btn-lg", icon = icon("cogs"))
          )
        ),
      ...
      # Query Method Tab
      tabItem(
        tabName = "query_method",
        fluidRow(
          box(
            width = 12,
            title = "Step 5: Query Method - Cell Type Enrichment",
            status = "primary",
            solidHeader = TRUE,
            
            div(
              class = "info-box",
              h4("ℹ️ About Query Method:"),
              p("The Query method measures summed region gene expression across cell types. 
                Higher positive z-scores indicate stronger enrichment, while higher negative z-scores indicate depletion.")
            ),
            
            hr(),
            
            actionButton("run_query_method_btn", "Run Query Method Analysis", class = "btn-custom btn-lg", icon = icon("search")),
            ...
            conditionalPanel(
              condition = "output.query_method_complete",
              h4("Enrichment Results:"),
              h5("Positive Enrichment"),
              plotOutput("query_bubble_plot_positive", height = "600px"),
              hr(),
              h5("Negative Enrichment"),
              plotOutput("query_bubble_plot_negative", height = "600px"),
              hr(),
              h4("Overlap with scRNA-seq genes:"),
              DTOutput("overlap_summary_table"),
              hr(),
              h4("Overlapping Gene List:"),
              DTOutput("overlap_genes_table"),
              downloadButton("download_overlap_genes", "Download Overlap Genes", class = "btn-custom"),
              hr(),
              DTOutput("query_results_table"),
              downloadButton("download_query_results", "Download Query Results", class = "btn-custom"),
              downloadButton("download_query_plot", "Download Plot", class = "btn-custom")
            )
          )
        )
      ),
      ...
      tabItem(
        ...
        fluidRow(
          box(
            width = 12,
            title = "Summary & Download Results",
            status = "success",
            solidHeader = TRUE,
            ...
            fluidRow(
              column(4,
                     downloadButton("download_all_results", "Download All Results (.zip)", class = "btn-custom btn-block")
              ),
              column(4,
                     downloadButton("download_report", "Generate Report (PDF)", class = "btn-custom btn-block")
              ),
              column(4,
                     actionButton("reset_analysis", "Reset Analysis", class = "btn-danger btn-block", icon = icon("refresh"))
              )
            )
          )
        )
...
server <- function(input, output, session) {
  ...
  observeEvent(input$start_btn, {
    updateTabItems(session, "tabs", "proteomics_upload")
  })
